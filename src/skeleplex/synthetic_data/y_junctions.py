...
def generate_y_junction(length_parent,
                        length_d1,
                        length_d2,
                        radius_parent,
                        radius_d1,
    ...
    skeleton = np.pad(skeleton,
                      pad_width= max_radius*2,
                      mode='constant',
                      constant_values=0)
    branch = skeleton.copy()

    # Draw the main branch
    origin = np.array([skeleton.shape[0] // 2, max_radius*2])  # Start at the middle of the image
    ...
    for i, point in enumerate([origin, p1, d1, d2]):
        ...
        draw_ellipsoid_at_point(branch,
                                point,
                                radii = (r_tip * np.random.uniform(1, 1.1),
    ...
    if use_gpu:
        ...
        distance_field = da.map_overlap(local_normalized_distance_gpu,
                                        branch_noisey_dask,
                                        max_ball_radius=30,
                                        depth=30).compute()
    else:
       ...
       distance_field =  da.map_overlap(local_normalized_distance,
                                        branch_noisey_dask,
                                        max_ball_radius=30,
                                        depth=30).compute()


    skeletonization_blurr = make_skeleton_blur_image(skeleton,
                                                      dilation_size=8,
                                                      gaussian_size=1.5, 
                                                      )
    skeletonization_target = skeletonization_blurr > 0.7
...
def random_parameters(
    ...
    return (length_parent, length_d1, length_d2,
            radius_parent, radius_d1, radius_d2,
            d1_angle, d2_angle,
            wiggle_factor, noise_magnitude,
            use_gpu)
